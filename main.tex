...
The central conclusion is the following. The later Gibbs-oriented paper \emph{does} recover the non-regularized horseshoe Gibbs updates for the global--local shrinkage block, but this is \emph{not} because the slab scale is fixed. Rather, it is because the paper changes the \emph{joint} prior structure by introducing an equivalent fictitious-data representation. This preserves the conditional law of $(\tau,\lambda)$ given $\beta$, while still producing the same regularized conditional distribution for $\beta \mid \tau,\lambda,\zeta$ as in the original regularized horseshoe. A prior on the slab scale remains perfectly reasonable from a Bayesian standpoint; what differs across the papers is primarily computational strategy and theoretical focus.
...
\label{eq:ns-block}
\end{equation}
This is exactly the ordinary global--local shrinkage block.
...
\]
is true, but it is true only because the \emph{joint prior} has been modified to make it true.
...
The Nishimura--Suchard paper often reads as if $\zeta$ were meant to be fixed. This is not the right interpretation.
...
\item \textbf{Ergodicity theory:} for their convergence proofs the paper assumes either fixed $\zeta$ or an upper-bounded support, because this keeps the drift/minorization arguments manageable.
\end{enumerate}
...
There is one sentence in Fan et al.\ that should be read with care. They say that when $b\to 0$ (that is, $\zeta\to 0$ in the unified notation), the coefficient is ``not penalized.'' This is inconsistent with their own shrinkage-factor definitions. From
\[
v_j(\zeta)=\left(\zeta^{-2}+u_j^{-2}\right)^{-1},
\]
it is immediate that $\zeta \to 0$ implies $v_j(\zeta)\to 0$, hence \emph{maximal} regularization rather than none. The formulas in the paper support the latter interpretation, so the quoted sentence should be treated as a verbal slip rather than as a substantive modeling difference.
...
\item Update $(\tau,\lambda)$ from the \emph{ordinary} horseshoe/global--local block
\[
...
\begin{enumerate}
\item It recovers the non-regularized horseshoe updates for the global--local shrinkage block $(\tau,\lambda)$.
...
\begin{quote}
The later Gibbs-oriented paper does recover the non-regularized horseshoe Gibbs updates for the \emph{global--local shrinkage block}, but not for the coefficient block, and not because the slab scale is fixed. The recovery is achieved by changing the \emph{joint prior structure} via a fictitious-data augmentation. A prior on the regularization parameter $\zeta$ is still a perfectly sensible Bayesian choice; the reasons some papers fix it are mainly expository, computational, or driven by the needs of ergodicity theory.
\end{quote}
